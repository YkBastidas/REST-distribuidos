                    ...
                    if action_type == 'REPLICATE':
                        ...
                        with socket.socket(
                            ...
                            if action_type.decode("utf-8") == "REPLICATE":
                                socket_replication.sendall(
                                    action)  # SEND ACTION (COMMIT OR ABORT) 1
                                action = socket_replication.recv(1024)  # RECEIVE COMMIT 2
                                print("Received", repr(action))
                                socket_replication.sendall(b"VOTE_REQUEST")     #SEND VOTE_REQUEST 3
                                vote = socket_replication.recv(
                                    1024)  # RECEIVE VOTE_COMMIT OR VOTE_ABORT 4
                                print("Received", repr(vote))
                                if vote.decode("utf-8") == "VOTE_COMMIT":
                                    socket_replication.sendall(b"GLOBAL_COMMIT")    #SEND GLOBAL_COMMIT 5
                                    ...
                                    socket_replication.send(file_data)  # ENVIO DE DATA 7
                                    file.close()
                                    print("Done Sending!")
                                    receive = socket_replication.recv(1024) #RECEIVE SUCCEED 8
                                else:
                                    socket_replication.sendall(b"GLOBAL_ABORT") # SEND ABORT 9
                                    receive = socket_replication.recv(1024) # RECEIVE FILED 10
                                socket_replication.close()
                                print("Received", repr(receive))  # RECEIVE REPLICATION OUTCOME
                                conn.sendall(receive)  # SEND OUTCOME TO APP SERVER (API)
                        ...
                        with socket.socket(
                            ...
                            if action_type.decode("utf-8") == "REPLICATE":
                                ...
                                action = socket_replicationB.recv(1024)  # RECEIVE COMMIT 2
                                print("Received", repr(action))
                                ...
                                if vote.decode("utf-8") == "VOTE_COMMIT":
                                    ...
                                    receive = socket_replicationB.recv(1024)    # RECEIVE GLOBAL 6
                                    ...
                                    file.close()
                                    print("Done Sending!")
                                    receive = socket_replicationB.recv(1024)    # RECEIVE SUCCEED 8
                                else:
                                    socket_replicationB.sendall(b"GLOBAL_ABORT") # SEND ABORT 9
                                    receive = socket_replicationB.recv(1024)    # RECEIVE FAILED 10
                        ...
                        with socket.socket(
                            ...
                            if action.decode("utf-8") == "A":
                                ...
                                print("Received", repr(receive))
                                root = os.path.dirname(__file__)
                                filename = os.path.join(root, "applicationServer" ,"objectsDatabaseRestore.json")
